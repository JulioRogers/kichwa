...
import Kichwa.ast as ast
from Kichwa.classes import (
    Boolean,
    Builtin,
    Float,
    Function,
...
from Kichwa.functions import BUILTIN

TRUE = Boolean(True)
FALSE = Boolean(False)
NULL = Null()

_NOT_A_FUNCTION = 'It is not a fshukcion: {}'
...
_WRONG_ARGS = 'Arguments wrongs: The fshukcion expected to receive types {} and receives {}'
_WRONG_OUTPUT = 'Output wrongs: The fshukcion expected to return type {} and return {}'
...
_INCOMPATIBLE_NULL_OPTERATION = 'Incompatible chusak operation: It is not possible to do the operation {} between a {} and {}'
...
TYPE_REGISTER_LITERAL: Dict[str, ObjectType] = {
    ...
    'fshukcion': ObjectType.FUNCTION,
    'chusak': ObjectType.INTEGER,
}
...
def evaluate(node: ast.ASTNode, env: Environment) -> Optional[Object]:
    ...
    if node_type == ast.Program:
        ...
        fshukcion = evaluate(node.fshukcion, env)
        assert fshukcion is not None
        if fshukcion.type() is ObjectType.ERROR:
            return fshukcion

        assert node.arguments is not None
        args = _evaluate_expression(node.arguments, env)

        if not _check_type_args_function(fshukcion, args):
            fshukcion = cast(Function, fshukcion)

            type_params = fshukcion.type_parameters
        ...
        return_fn = _apply_function(fshukcion, args)

        if type(fshukcion) == Builtin:
            return return_fn
        elif type(fshukcion) == Function \
                and _check_type_out_function(fshukcion, return_fn):
            return return_fn
        elif type(fshukcion) == Error:
            return return_fn
        else:
            fshukcion = cast(Function, fshukcion)
            ...
            return _new_error(_WRONG_OUTPUT, [
                fshukcion.type_output, TYPE_REGISTER_OBJECT[return_fn.type()]])
...
def _evaluate_infix_expression(operator: str,
    ...
    if left.type() == ObjectType.INTEGER \
        ...
        if left.inspect() == 'chusak' or right.inspect() == 'chusak':
            return NULL
