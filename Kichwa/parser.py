...
from Kichwa.lexer import Lexer
from Kichwa.tokens import (
    Token,
    TokenType
)
...
class Parser:
    ...
    def _parse_call(self, fshukcion: Expression) -> Call:
        assert self._current_token is not None
        call = Call(self._current_token, fshukcion)
    ...
    def _parse_function(self) -> Optional[Function]:
        assert self._current_token is not None
        fshukcion = Function(token=self._current_token)

        if not self._expected_token(TokenType.IDENT):
            return None

        fshukcion.parameters, fshukcion.type_parameters, fshukcion.type_output = self._parse_function_parameters()

        if not self._expected_token(TokenType.LBRACE):
            return None

        fshukcion.body = self._parse_block()
